...
describe('AlertController', () => {
  it('returns repository-scoped actionable alerts without controller-side wallet filtering', async () => {
    ...
    const originalListActionableTriggers = triggerRepo.listActionableTriggers.bind(triggerRepo);
    triggerRepo.listActionableTriggers = async (walletId: WalletId) => {
    ...
    expect(triggerRepo.lastListedWalletId).toBe(requestedWalletId);
  });
