export const TRIGGER_REPOSITORY = 'TRIGGER_REPOSITORY';
export const EXECUTION_REPOSITORY = 'EXECUTION_REPOSITORY';
export const EXECUTION_HISTORY_REPOSITORY = 'EXECUTION_HISTORY_REPOSITORY';
export const EXECUTION_PREPARATION_PORT = 'EXECUTION_PREPARATION_PORT';
export const EXECUTION_SUBMISSION_PORT = 'EXECUTION_SUBMISSION_PORT';
export const SUPPORTED_POSITION_READ_PORT = 'SUPPORTED_POSITION_READ_PORT';
export const SWAP_QUOTE_PORT = 'SWAP_QUOTE_PORT';
export const CLOCK_PORT = 'CLOCK_PORT';
export const ID_GENERATOR_PORT = 'ID_GENERATOR_PORT';
export const MONITORED_WALLET_REPOSITORY = 'MONITORED_WALLET_REPOSITORY';
export const REGIME_ENGINE_EVENT_PORT = 'REGIME_ENGINE_EVENT_PORT';
export const CURRENT_SR_LEVELS_PORT = 'CURRENT_SR_LEVELS_PORT';
export const OBSERVABILITY_PORT = 'OBSERVABILITY_PORT';
export const PG_BOSS_INSTANCE = 'PG_BOSS_INSTANCE';
export const SR_LEVELS_POOL_ALLOWLIST = 'SR_LEVELS_POOL_ALLOWLIST';
export const RECONCILIATION_JOB_PORT = 'RECONCILIATION_JOB_PORT';
export const PRICE_PORT = 'PRICE_PORT';
export const SR_LEVELS_READ_PORT = 'SR_LEVELS_READ_PORT';
export const INSIGHTS_API_KEY = 'INSIGHTS_API_KEY';
export const WALLET_CHALLENGE_REPOSITORY = 'WALLET_CHALLENGE_REPOSITORY';
export const DB = 'DB';
export const REGIME_READ_PORT = 'REGIME_READ_PORT';
export const REGIME_POOL_ALLOWLIST = 'REGIME_POOL_ALLOWLIST';
export const SR_THESES_READ_PORT = 'SR_THESES_READ_PORT';
export const SR_THESES_POOL_ALLOWLIST = 'SR_THESES_POOL_ALLOWLIST';
export const POLICY_INSIGHTS_READ_PORT = 'POLICY_INSIGHTS_READ_PORT';
export const PLAN_REPOSITORY = 'PLAN_REPOSITORY';
export const REGIME_PLAN_PORT = 'REGIME_PLAN_PORT';
export const REGIME_PLAN_REQUEST_CONFIG = 'REGIME_PLAN_REQUEST_CONFIG';
export const EVIDENCE_READ_PORT = 'EVIDENCE_READ_PORT';
