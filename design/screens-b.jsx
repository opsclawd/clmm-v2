  ...
  return (
    <div className="clmm-screen">
      ...
      <div className="clmm-scroll" style={{ padding: "14px 20px 20px" }}>
        ...
        {/* ─── Market Thesis Card (summary text only) ─── */}
        <div className="clmm-card" style={{ marginTop: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <Icon.info className="ic" style={{ color: "var(--fg-3)" }}/>
            <div className="clmm-label" style={{ fontSize: 10 }}>Market thesis</div>
          </div>
          <div style={{ fontSize: 13.5, color: "var(--fg-1)", lineHeight: 1.55, fontWeight: 500 }}>
            Bearish swing, trend continuation. SOL is consolidating beneath the green resistance zone after a failed breakout. A close below the blue zone would confirm the next leg down.
          </div>
        </div>

        {/* ─── S/R Levels Card — grouped, with thesis details inside each group ─── */}
        <div className="clmm-card" style={{ marginTop: 14 }}>
          ...
          {[
            {
              kind: "resistance",
              bias: "Bearish swing",
              trigger: { label: "Break above", price: "$88.30", outcome: "invalidates bearish thesis" },
              invalidation: { label: "Close below", price: "$85.00", outcome: "confirms supply zone" },
              source: "morecryptoonline",
              timeframe: "4h",
              setup: "swing · trend continuation",
              note: "Green-zone resistance cluster · 86.37–88.30. Strong supply seen on prior tests.",
              levels: [
                { kind: "resistance", p: "$87.33", tone: "breach" },
                { kind: "resistance", p: "$89.40", tone: "breach" },
              ],
            },
            {
              kind: "support",
              bias: "Bearish swing",
              trigger: { label: "Break below", price: "$80.00", outcome: "C-wave down activates" },
              invalidation: { label: "Hold above", price: "$83.00", outcome: "Sunday-low defense" },
              source: "morecryptoonline",
              timeframe: "4h",
              setup: "swing · trend continuation",
              note: "Blue-zone support cluster · 79–81. Sunday low at $83.00 is first defense.",
              levels: [
                { kind: "support", p: "$80.00", tone: "safe" },
                { kind: "support", p: "$83.00", tone: "safe" },
              ],
            },
          ].map((g, gi) => (
            <div key={gi} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <span className="clmm-chip warn" style={{ height: 20, padding: "0 8px", fontSize: 10 }}>
                  <span className="d"/>{g.bias}
                </span>
                <span className="mono" style={{ fontSize: 10, color: "var(--fg-4)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                  {g.kind === "resistance" ? "Resistance" : "Support"} cluster
                </span>
              </div>
              ...
              {g.levels.map((lv, li) => (
                <div key={li} style={{
                  ...
                  padding: "8px 0",
                  borderTop: li === 0 ? "1px solid var(--line-1)" : "1px solid var(--line-1)",
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span className="clmm-chip" style={{
                      height: 22, padding: "0 8px", fontSize: 10,
                      color: lv.tone === "breach" ? "var(--breach-ink)" : "var(--safe-ink)",
                      borderColor: lv.tone === "breach" ? "var(--breach-br)" : "var(--safe-br)",
                      background: lv.tone === "breach" ? "var(--breach-bg)" : "var(--safe-bg)",
                    }}>{lv.kind === "resistance" ? "Resist" : "Support"}</span>
                    <span className="mono tnum" style={{ fontSize: 14, fontWeight: 600 }}>{lv.p}</span>
                  </div>
                </div>
              ))}
              ...
              <div style={{
                marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--line-1)",
                fontSize: 12, color: "var(--fg-2)", lineHeight: 1.55,
              }}>
                <div>
                  <span style={{ color: "var(--breach-ink)", fontWeight: 600 }}>Trigger</span>
                  <span style={{ color: "var(--fg-3)" }}> · {g.trigger.label} </span>
                  <span className="mono tnum" style={{ color: "var(--fg-1)" }}>{g.trigger.price}</span>
                  <span style={{ color: "var(--fg-3)" }}> · {g.trigger.outcome}</span>
                </div>
                <div style={{ marginTop: 4 }}>
                  <span style={{ color: "var(--safe-ink)", fontWeight: 600 }}>Invalidation</span>
                  <span style={{ color: "var(--fg-3)" }}> · {g.invalidation.label} </span>
                  <span className="mono tnum" style={{ color: "var(--fg-1)" }}>{g.invalidation.price}</span>
                  <span style={{ color: "var(--fg-3)" }}> · {g.invalidation.outcome}</span>
                </div>
              </div>
              ...
              <div style={{
                ...
                <span>Source · <span style={{ color: "var(--fg-2)" }}>{g.source}</span></span>
                <span>TF · <span style={{ color: "var(--fg-2)" }}>{g.timeframe}</span></span>
                <span>Setup · <span style={{ color: "var(--fg-2)" }}>{g.setup}</span></span>
              </div>
            </div>
          ))}
        </div>
