...
export const breachEpisodes = pgTable(
  'breach_episodes',
  {
  ...
  (table) => [
    ...
    check('breach_episodes_status_check', sql`${table.status} in ('open', 'closed')`),
    check('breach_episodes_consecutive_count_min_check', sql`${table.consecutiveCount} >= 1`),
    check(
...
export const exitTriggers = pgTable(
  'exit_triggers',
  {
