...
export function evaluateRetryEligibility(attempt: ExecutionAttempt): RetryEligibility {
  const { lifecycleState, completedSteps } = attempt;

  if (lifecycleState.kind === 'partial') {
    return {
      kind: 'ineligible',
      reason: 'partial completion: one or more chain steps confirmed; full replay is forbidden',
    };
  }

  if (lifecycleState.kind === 'confirmed' || lifecycleState.kind === 'abandoned') {
    return {
      kind: 'ineligible',
  ...
  if (lifecycleState.kind === 'awaiting-signature' || lifecycleState.kind === 'submitted') {
    return {
      kind: 'ineligible',
